...
self.addEventListener( "fetch", (event: any) => {
	...
	logger_service_worker (`****************************** accessHref = ${accessHref}`, event)
	...
	if (!reqUrl ) {
		...
		const _site = clientDomainPool[clientDomainPool.length - 1]
		reqUrl = accessHref.replace ( location.origin, _site.siteOrigin.origin )
		gateway = _site.node
	...
	//		has reqUrl
		...
		if (event.clientId) {
			...
			logger_service_worker (`############################ event.clientId is NULL`)
			logger_service_worker
			logger_service_worker (`############################ event.clientId is NULL`)
		}
	}
...
self.addEventListener('install', event => {
	console.log('API/SW V1 installing…!')
  
	//	@ts-ignore
	event.waitUntil(
		logger_service_worker(`API/SW V1 waitUntil SUCCESS!`)
	  	//caches.open('static-v1').then(cache => cache.add('/cat.svg'))
	)
})
  
self.addEventListener('activate', event => {
	logger_service_worker('API/SW V1 now ready to handle fetches!')
})
