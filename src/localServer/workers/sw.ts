...
const checkBlockList = (url: string ) => {
	if (blocklist.length ) {
...
const returnCacheFirst = (event) => {
	workerClientID = event.resultingClientId||event.clientId
...
const conetCoreFileList: RegExp[] = [
	new RegExp(`${location.origin}/static/`)
]

const checkCoreFile = (url: string) => {
	let ret = false
	conetCoreFileList.forEach (n => {
		if (n.test (url)) {
			ret = true
		}
...
self.addEventListener( "fetch", async (event: any) => {
	const accessHref: string = event.request.url

	//		CONET Platform CORE or Browser navigate to new location
	if (event.request.mode === 'navigate'|| checkCoreFile (accessHref)) {
		const matchLocal = new RegExp (`${location.origin}/api`)
		if (!matchLocal.test (accessHref)) {
			return returnCacheFirst (event)
		}
		logger_service_worker (` ****************** %%%%%%%%%%%%%%%%%%%%%%% accessHref pre process= ${accessHref} resultingClientId [${ event.resultingClientId }] &&&&&&&&&&&&&&&&&&&&&& ****************** ` )
	}

	//		URL in Blacklist
	if (checkBlockList(accessHref)) {
	...
	if (!reqUrl ) {
		...
		//	
		if (workerClientID && clientId.clientId === workerClientID || siteMatch.test (accessHref)) {
			logger (`>>>>>>>>>>>>>>>>>>>*************** !reqUrl workerClientID = `,workerClientID)
			workerClientID = clientId.clientId
			logger_service_worker (` >>>>>>>>>>>>>>>>>>>*************** PASS accessHref = ${accessHref} workerClientID [${workerClientID}] resultingClientId [${event.resultingClientId}] clientId [${event.clientId}]for CORE`)
			return
		}

		logger_service_worker (`****************************** accessHref = ${accessHref} workerClientID [${workerClientID}] clientId.clientId [${ clientId.clientId }]`)
		reqUrl = accessHref
		const accessHrefUrl = new URL (accessHref)

		if (accessHrefUrl.origin === location.origin) {

			let siteOrigin = ''

			if (clientId.clientId) {
				const index = clientDomainPool.findIndex(n => n.clientId.resultingClientId === clientId.clientId)
				if (index > -1) {
					siteOrigin = clientDomainPool[index].siteOrigin.origin
					//gateway = clientDomainPool[index].node
				}
			}

			if (!siteOrigin) {
				const cmd: IWorker_command = await getDomain(clientId.clientId)

				if (cmd.err) {
					logger_service_worker (`############################ can't find DOMAIN !`)
					logger_service_worker (reqUrl, clientId.clientId)
					logger_service_worker (`############################ can't find DOMAIN !`)
					return err_response_network_err()
				}

				const resObj = cmd.data[1]


				siteOrigin = resObj.domain
				//gateway = resObj.node
			
			}
			siteOrigin = siteOrigin.replace(/\/$/,'')
			reqUrl = accessHref.replace ( /api\/\//, '' )
			reqUrl = reqUrl.replace ( location.origin, siteOrigin )
	...
	return event.respondWith(( async () => {
		...
		if ( urlData.method !== 'GET') {
			urlData.json = await readRequestBody(event.request)
			logger (`************ URL ${ reqUrl } method [${urlData.method}] !== 'GET' has request body `)
			logger (urlData.json)
			logger (`************ URL ${ reqUrl } method [${urlData.method}] !== 'GET' has request body `)
			if (/https:\/\/www.youtube.com\/youtubei\/v1\//.test(reqUrl)) {
				try {
					//	
					const k = JSON.parse
					//	@ts-ignore
					(urlData.json)
					const client = k.context.client
					if (client) {
						client.mainAppWebInfo.graftUrl = client.originalUrl = siteOrigin.href
					}
					//	@ts-ignore

					urlData.json = JSON.stringify(k)
					logger (k)
				} catch (ex) {
		...
		// if (/googlevideo.com/.test(siteOrigin.hostname) && /\/videoplayback/i.test(siteOrigin.pathname)) {
		// 	gateway =  clientDomainPool[clientDomainPool.length - 1].node
		// 	logger (gateway)
		// }
		...
		const body = buffer.Buffer.from(bb,'binary').buffer
...
const logger_service_worker = (...argv: any ) => {
    const date = new Date ()
    const dateStrang = `%c [Seguro-worker INFO ${ date.getHours() }:${ date.getMinutes() }:${ date.getSeconds() }:${ date.getMilliseconds ()}]`
...
listenChannel.addEventListener('message', e => {
	...
	if (cmd.cmd === 'READY') {
		cmd.data = [workerClientID]
		workerIsReady = true
		return channelServiceWroker.postMessage(JSON.stringify(cmd))
	}
	...
	if (index < 0) {
		return logger (`Service listenChannel.addEventListener('message' ERROR listenChannel [toServiceWroker] got unknow UUID message!`, cmd)
	}
...
const sendUrlToWorker: (urlData: urlData, headers: Object, gateway: Object|null) => Promise<IWorker_command> = (urlData, headers, gateway) => {
	return new Promise ( resolve => {
		...
		channelServiceWroker.postMessage(JSON.stringify(cmd))
		backGroundPool.push ({
			_promise: resolve,
			cmd,
			uuid: cmd.uuid
...
const storageDomain = (domain: URL,  gateway: any,  id: string) =>  {
...
const getDomain:(id: string)=> Promise<IWorker_command> = (id) => new Promise (resolve => {
		const cmd: IWorker_command = {
			cmd: "getDomain",
			uuid: getUUIDv4(),
			data: [id]
...
const getWorkerClientID: () =>Promise<IWorker_command> = () => new Promise (resolve => {
	const cmd: IWorker_command = {
		cmd: 'getWorkerClientID',
		uuid: getUUIDv4(),
		data: []
	}
	channelServiceWroker.postMessage(JSON.stringify(cmd))
...
const checkUpdate = (request: Request ) => {
	return new Promise (async resolve => {
		...
		const cmd = await sendUrlToWorker(urlData, [], null)
		if (cmd.err) {
			return resolve(null)
		}
		return resolve(cmd.data)
	})
}

self.addEventListener('install', event => {
	logger_service_worker('API/SW V1 installing…!')
	//	@ts-ignore
	self.skipWaiting()
	...
	// 	// caches.open(expectedCaches).then(
	// 	// 	cache => {
	// 	// 		// Add all the assets in the array to the 'MyFancyCacheName_v1'
	// 	// 		// `Cache` instance for later use.
	// 	// 		const listenInitFinishedFromWorker = e => {

	// 	// 			listenChannel.removeEventListener ('message', listenInitFinishedFromWorker)
	// 	// 			let cmd: IWorker_command
	// 	// 			try {
	// 	// 				cmd = JSON.parse(e.data)
	// 	// 			} catch (ex) {
	// 	// 				logger (`Service worker INIT listening toServiceWroker BroadcastChannel JSON parse Error`)
	// 	// 				return logger (e)
	// 	// 			}
	// 	// 			if (cmd.cmd === 'READY') {
	...
	// 	// 			}
	// 	// 			logger_service_worker('Service Worker caches.open SUCCESS', CACHED_URLS)
	// 	// 		}
	// 	// 		listenChannel.addEventListener('message', listenInitFinishedFromWorker)
	// 	// 	}
	// 	// )
	// )
...
self.addEventListener('activate', event => {
	...
	event.waitUntil(
		...
		// 	keys.map( key => {
		// 		if (expectedCaches !== key) {
		// 			logger (`Service Worker activate caches delete key`, key)
